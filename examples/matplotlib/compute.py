...
def compute_xs_gt(mu_L, delta_mu_L, g, t, erxs_coeff_np):
    L = 7
    ...
    for l in range(L + 1):
        s += (2 * l + 1) / delta_mu_L * erxs_coeff_np[l,g,t] * legendreP(l, mu_L)
    #print('s',s)
    return s

# Computes the cross section matrix for all T's and G's
def compute_xs_matrix(mu_L, delta_mu_L, G, T, erxs_coeff_np):
    temp = np.zeros((G, T))
    for t in range(T):
        for g in range(G):
            temp[g,t] = compute_xs_gt(mu_L,delta_mu_L,g,t,erxs_coeff_np)
    #print(temp)
    return temp
